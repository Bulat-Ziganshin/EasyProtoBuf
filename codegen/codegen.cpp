...
void generator(FileDescriptorSet &proto)
{
    ...
    for (auto message_type: file.message_type)
    {
        ...
        for (auto field: message_type.field)
        {
            ...
            if (field.label == FieldDescriptorProto::LABEL_REPEATED) {
                ...
                if(! option.no_has_fields) {
                    extra_parameter = myformat(", &has_{0}", field.name);
                }
                get_call = myformat("pb.get_{0}(&{1}{2})", pbtype_str, field.name, extra_parameter);
            }

            decode_cases += myformat("            case {}: {}; break;\n", std::to_string(field.number), get_call);
        ...
        if(! option.no_class) {
            std::cout << myformat(CLASS_TEMPLATE, message_type.name, fields_defs, has_fields_defs,
        ...
        if(! option.no_decoder) {
            std::cout << myformat(DECODER_TEMPLATE, message_type.name, decode_cases, check_required_fields);
        }
    }
}
